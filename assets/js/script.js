  ...
  function productCategoryFilter() {
    ...
    function renderCard(product) {
      ...
      var cardPoints = [primaryPoint].map(function (feature) {
        ...
        '<article class="product__filter-card" data-category="' + product.category + '" style="--product-accent:' + accent + '; --product-soft:' + soft + '; height: 100%;">',
        '<div class="tt-product-card" style="height: 100%; box-sizing: border-box;">',
        '<div class="tt-card-top" style="margin-bottom: 20px;">',
        '<div class="tt-card-icon" style="flex-shrink: 0;">',
        getLogoMarkup(product),
        '</div>',
        '<div class="tt-card-badges">',
        '<span class="tt-card-orders" style="color:var(--product-accent); border-color:var(--product-accent); background:color-mix(in srgb, var(--product-accent) 15%, transparent);"><i class="fas fa-tag"></i> ' + product.badge + '</span>',
        '</div>',
        '</div>',
        '<div class="tt-card-content">',
        '<h3 class="tt-card-title" style="margin-bottom: 12px;"><a href="checkout.html" style="color:#fff;text-decoration:none;">' + displayTitle + '</a></h3>',
        '<ul style="margin: 0 0 20px 0; padding: 0; list-style: none; color: #a1a1aa; font-size: 0.9rem; line-height: 1.4;">',
        cardPoints.replace(/class="product__feature-item"/g, 'class="product__feature-item" style="position:relative; padding-left:18px; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden;"').replace(/<li /g, '<li style="position:relative; margin-bottom: 8px;" ').replace(/>/g, '><span style="position:absolute; left:0; color:var(--product-accent, #10b981); font-weight:900;">✓</span> '),
        '</ul>',
        '<div class="tt-card-price-action">',
        '<span class="tt-card-price">' + product.price + ' <span style="font-size:0.8rem; font-weight:normal; color:#888;">' + product.plan + '</span></span>',
        '<div class="product__card-actions" style="display:flex; gap:10px; margin-top: 10px;">',
        '<button class="product__card-action-btn product__card-action-btn--icon product__card-action-btn--wishlist' + (isWishlisted ? ' is-active' : '') + '" style="width:44px; height:44px; padding:0; background:rgba(255,255,255,0.05); color:#fff; border:1px solid rgba(255,255,255,0.1); border-radius:6px; transition:0.3s; flex-shrink:0; cursor:pointer;" type="button" data-product-key="' + productKey + '" data-product-action="toggle-wishlist"><i class="fa fa-heart"></i></button>',
        '<button class="thm-btn thm-btn-two tt-btn-custom product__card-action-btn" type="button" data-product-key="' + productKey + '" data-product-action="add-to-cart" style="flex:1; border:none; cursor:pointer;">Add to Cart</button>',
        '</div>',
        '</div>',
        '</div>',
        '</div>',
